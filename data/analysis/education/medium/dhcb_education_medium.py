...
dhcb_education_medium1 = dhcb_education[
    (dhcb_education['Total.Population.of.Village'] >= 1000) & 
    (dhcb_education['Total.Population.of.Village'] <= 2500)
].copy()

print(f"\nMedium villages (1000-2500 pop): {dhcb_education_medium1.shape[0]} rows")

# --- 4. Scale Numerical Features ---
# Select columns 3:29 (2:29 in Python 0-indexing)
scaling_cols = dhcb_education_medium1.columns[2:29]

print(f"\nScaling {len(scaling_cols)} columns for clustering...")

scaler = StandardScaler()
dhcb_education_medium1_scaled = pd.DataFrame(
    scaler.fit_transform(dhcb_education_medium1[scaling_cols]), 
    columns=scaling_cols
)

# Remove columns with zero variance (columns 18, 25 in R = indices 17, 24 in Python)
# No "Total.Medicine.College" and "Total.School.For.Disabled" in medium villages
cols_to_drop = []
for idx in [17, 24]:
    if idx < len(dhcb_education_medium1_scaled.columns):
        cols_to_drop.append(dhcb_education_medium1_scaled.columns[idx])

dhcb_education_medium1_scaled1 = dhcb_education_medium1_scaled.drop(columns=cols_to_drop)

print(f"After removing zero-variance columns: {dhcb_education_medium1_scaled1.shape}")
print(f"Missing values: {dhcb_education_medium1_scaled1.isna().sum().sum()}")
...
wss_plot(dhcb_education_medium1_scaled1, title='Elbow Plot for Medium Villages (Education)')
...
dhcb_education_medium1['Clusters'] = kmeans_medium.fit_predict(dhcb_education_medium1_scaled1)

print("Clustering complete. 'Clusters' column added.")

# --- 7. Aggregate Results by Cluster ---
# Exclude columns 1, 2, 20, 27 (indices 0, 1, 19, 26 in Python)
cols_to_exclude = [0, 1, 19]
if dhcb_education_medium1.shape[1] > 27:
    cols_to_exclude.append(26)

aggr_cols = [col for idx, col in enumerate(dhcb_education_medium1.columns) 
             if idx not in cols_to_exclude and col != 'Clusters']

aggr = dhcb_education_medium1.groupby('Clusters')[aggr_cols].mean(numeric_only=True)
...
dhcb_education_medium1.to_csv("00_dhcb_education_medium1.csv", index=False)
aggr.to_csv("00_dhcb_education_medium11.csv")

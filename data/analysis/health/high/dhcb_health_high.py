...
print(f"\nHigh villages (>2500 pop): {dhcb_health_high.shape[0]} rows")

# --- 4. Scale Numerical Features ---
# Select columns 3:51 (2:51 in Python 0-indexing)
scaling_cols = dhcb_health_high.columns[2:51]

print(f"\nScaling {len(scaling_cols)} columns for clustering...")

scaler = StandardScaler()
dhcb_health_high_scaled = pd.DataFrame(
    scaler.fit_transform(dhcb_health_high[scaling_cols]), 
    columns=scaling_cols
)

print(f"Scaled data shape: {dhcb_health_high_scaled.shape}")
print(f"Missing values in scaled data: {dhcb_health_high_scaled.isna().sum().sum()}")

# Remove columns with zero variance for high villages
# Columns to remove (R indices): 42, 43, 44
# Python indices: 41, 42, 43
# These include:
# - Mobile.Health.Clinic and related
cols_to_drop = []
drop_indices = [41, 42, 43]
for idx in drop_indices:
    if idx < len(dhcb_health_high_scaled.columns):
        cols_to_drop.append(dhcb_health_high_scaled.columns[idx])

dhcb_health_high_scaled1 = dhcb_health_high_scaled.drop(columns=cols_to_drop)

print(f"After removing zero-variance columns: {dhcb_health_high_scaled1.shape}")
print(f"Columns in scaled data: {dhcb_health_high_scaled1.columns.tolist()}")
print(f"Missing values: {dhcb_health_high_scaled1.isna().sum().sum()}")
...
wss_plot(dhcb_health_high_scaled1, title='Elbow Plot for High Villages (Health)')
...
dhcb_health_high['Clusters'] = kmeans_high.fit_predict(dhcb_health_high_scaled1)

print("Clustering complete. 'Clusters' column added.")
print(f"Columns in final dataframe: {dhcb_health_high.columns.tolist()}")

# --- 7. Aggregate Results by Cluster ---
# Exclude columns 1, 2, 44, 45, 46 (indices 0, 1, 43, 44, 45 in Python)
cols_to_exclude = [0, 1, 43, 44, 45]

aggr_cols = [col for idx, col in enumerate(dhcb_health_high.columns) 
             if idx not in cols_to_exclude and col != 'Clusters']

aggr = dhcb_health_high.groupby('Clusters')[aggr_cols].mean(numeric_only=True)
...
dhcb_health_high.to_csv("dhcb_health_high.csv", index=False)
aggr.to_csv("dhcb_health_high1.csv")
